...
export default function AdminDashboard() {
  ...
  function renderWalkerSelect(walk: Walk) {
    const assignableWalkers = walkers.filter(w => w.active || w.id === walk.walker_id);
    const availIds = getAvailableWalkerIds(walk.scheduled_date);
    const available = assignableWalkers.filter(w => availIds.has(w.id));
    const others = assignableWalkers.filter(w => !availIds.has(w.id));
          ...
          {available.length > 0 && others.length > 0 ? (
            <>
              <optgroup label="Available">
                {available.map(w => <option key={w.id} value={w.id}>{w.full_name}{!w.active ? ' (suspended)' : ''}</option>)}
              </optgroup>
              <optgroup label="Others">
                {others.map(w => <option key={w.id} value={w.id}>{w.full_name}{!w.active ? ' (suspended)' : ''}</option>)}
              </optgroup>
            </>
          ) : (
            assignableWalkers.map(w => <option key={w.id} value={w.id}>{w.full_name}{!w.active ? ' (suspended)' : ''}</option>)
          )}
        </select>
